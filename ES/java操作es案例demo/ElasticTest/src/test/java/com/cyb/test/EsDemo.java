...
public class EsDemo {
	...
	public void test6() throws UnknownHostException {
		...
		for(MultiGetItemResponse item:response) {
			...
			if(gr!=null&&gr.isExists()) {//如果gr不为空，并且有值
...
//boolQuery 组合查询
@Test
public void test38() throws UnknownHostException{
	//1、指定es集群  cluster.name 是固定的key值，my-application是ES集群的名称
	Settings settings = Settings.builder().put("cluster.name", "my-application").build();
	//2.创建访问ES服务器的客户端
	TransportClient client = new PreBuiltTransportClient(settings)
			.addTransportAddress(new TransportAddress(InetAddress.getByName("192.168.1.94"), 9300));
	//查询interets字段中必须含有changge并不能含有lvyou 或者address含有bei jing 然后过滤生日都大于1990-01-01的文档
	QueryBuilder builder = QueryBuilders.boolQuery()
			.must(QueryBuilders.matchQuery("interets", "changge"))
			.mustNot(QueryBuilders.matchQuery("interets", "lvyou"))
...
//constantScoreQuery  不计相关度查询
@Test
public void test39() throws UnknownHostException{
	//1、指定es集群  cluster.name 是固定的key值，my-application是ES集群的名称
	Settings settings = Settings.builder().put("cluster.name", "my-application").build();
	//2.创建访问ES服务器的客户端
	TransportClient client = new PreBuiltTransportClient(settings)
			.addTransportAddress(new TransportAddress(InetAddress.getByName("192.168.1.94"), 9300));
	//constantScoreQuery不计相关度查询
